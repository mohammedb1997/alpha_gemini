...
function lNextQ() {
  ...
  document.getElementById('lq-text').innerText = LG.activeQ.q;
  document.getElementById('lq-ans').innerText = '✅ الإجابة: ' + LG.activeQ.a;
  document.getElementById('lq-ans').style.display = 'none'; clearInterval(LG.timerInt);
  lHexClick(LG.activeCell);
}

function lMark(team) {
  if (LG.activeCell === null) return; LG.owner[LG.activeCell] = team; LG.pts[team]++; clearInterval(LG.timerInt); lCloseModal();
  renderHexBoard(document.getElementById('local-board'), LG.letters, LG.owner, null, lHexClick); lUpdateUI();
...
function lShowWinner(team) {
  LG.rndWins[team]++; lUpdateUI(); const nm = team === 'a' ? LG.teamA : LG.teamB; const col = team === 'a' ? 'var(--A)' : 'var(--B)';
...
function connectJudge() {
  ...
  JS.peer.on('error', e => { showToast('خطأ في الاتصال: ' + e.type, 'error'); if (currentServerIndex < PEER_SERVERS.length - 1) { currentServerIndex++; connectJudge(); } else { currentServerIndex = 0; JS.retryTimeout = setTimeout(connectJudge, 3000); } });
...
function jInitBoard() {
  JS.letters = randLetters(); JS.owner = Array(N).fill(null); JS.activeCell = null; JS.activeQ = null; JS.phase = 'idle'; JS.bzWinner = null; JS.pts = { a: 0, b: 0 };
...
function jCorrect(team) {
  if (JS.activeCell === null) return; JS.owner[JS.activeCell] = team; JS.pts[team]++; jClearTimer(); JS.phase = 'idle'; JS.bzWinner = null; JS.activeCell = null;
  renderHexBoard(document.getElementById('judge-board'), JS.letters, JS.owner, null, jHexClick); jUpdateScores(); jRenderBz();
  ...
  if (bfsWin(JS.owner, team)) { jShowWinner(team); return; } if (JS.owner.every(o => o)) jShowWinner(JS.pts.a >= JS.pts.b ? 'a' : 'b');
}
...
function jShowWinner(team) {
  JS.rndWins[team]++; jUpdateScores(); const nm = team === 'a' ? JS.teamA : JS.teamB; const col = team === 'a' ? 'var(--A)' : 'var(--B)';
  ...
  document.getElementById('judge-win').classList.add('on'); confetti(team); jBcast({ type: 'round_win', team, rndWins: JS.rndWins });
...
function updatePlayerCodeBadge() {
  ...
  if (elWait) elWait.textContent = '🎮 كود: ' + code;
  if (elBz) elBz.textContent = '🎮 كود: ' + code;
}
...
function connectPlayer(code) {
  ...
  pPeer.on('error', e => { showToast('خطأ في الاتصال: ' + e.type, 'error'); if (currentServerIndex < PEER_SERVERS.length - 1) { currentServerIndex++; connectPlayer(code); } else { currentServerIndex = 0; pRetryTimeout = setTimeout(() => connectPlayer(code), 3000); } });
...
function dpApply(s) {
  ...
  dpBrd = s.board || [];
  const letters = dpBrd.map(c => c.letter); const owner = dpBrd.map(c => c.owner);
  renderHexBoard(document.getElementById('display-board'), letters, owner, s.activeCell, null);
  if (s.showAnswer && s.activeQ) { document.getElementById('dp-ans').innerText = '✅ الإجابة: ' + s.activeQ.a; document.getElementById('dp-ans').style.display = 'block'; } else { document.getElementById('dp-ans').style.display = 'none'; }
  if (s.activeQ) dpPendingQuestion = s.activeQ.q;
}
...
function setDiff(btn, mode) {
  const container = mode === 'j' ? document.getElementById('jdiff-filter') : document.getElementById('ldiff-filter');
  container.querySelectorAll('.diff-btn').forEach(b => b.className = 'diff-btn');
...
function setCat(btn, mode) {
  const container = mode === 'j' ? document.getElementById('jcat-filter') : document.getElementById('lcat-filter');
...
function setLiveDiff(btn) {
  const container = document.getElementById('j-live-diff');
  container.querySelectorAll('.diff-btn').forEach(b => b.className = 'diff-btn');
  const d = btn.dataset.diff;
  btn.classList.add('active-' + (d === 'all' ? 'all' : d));
  LIVE_DIFF = d;
...
function lShowDiffBadge(q) {
  const badge = document.getElementById('lq-diff-badge');
  const catTag = document.getElementById('lq-cat-badge');
...
function renderTimerCircle(container, seconds, totalSeconds, color) {
  ...
  container.innerHTML = `
    <svg class="timer-circle" viewBox="0 0 64 64">
      <circle class="bg" cx="32" cy="32" r="${r}" fill="none" stroke-width="5" />
      <circle class="fg" cx="32" cy="32" r="${r}" fill="none" stroke-width="5"
        stroke="${color || 'var(--GOLD)'}"
        stroke-dasharray="${circumference}"
        stroke-dashoffset="${circumference * (1 - progress)}"
        transform="rotate(-90 32 32)" />
      <text x="32" y="36" text-anchor="middle" fill="white" font-size="18" font-weight="900">${seconds}</text>
    </svg>`;
}
...
jRenderBz = function () {
  ...
  if (JS.bzWinner) {
    const col = JS.bzWinner.team;
    const color = col === 'a' ? 'var(--A)' : 'var(--B)';
    const tn = col === 'a' ? JS.teamA : JS.teamB;
    const timerHTML = JS.timerSecs > 0 ? `<div id="judge-timer-circle"></div>` : '';
    bz.innerHTML = `
      <div class="bzwin bzw${col}">
        <div class="bzname">⚡ ${JS.bzWinner.name}</div>
        <div class="bzsub">${tn}</div>
        <div class="bzcount">${JS.timerSecs}</div>
        ${timerHTML}
