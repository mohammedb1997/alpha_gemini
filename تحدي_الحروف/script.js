...
function randLetters() {
  return Object.keys(QBANK).sort(() => Math.random() - .5).slice(0, N);
}
...
function pickQ(letter) {
  ...
  const qs = QBANK[letter];
  if (!qs || !qs.length) return null;
  if (!UQ[letter]) UQ[letter] = new Set();
  const u = UQ[letter];
  if (u.size >= qs.length) u.clear();
  let idx;
  do { idx = Math.floor(Math.random() * qs.length); } while (u.has(idx));
  u.add(idx);
  return qs[idx];
}
...
let localChannel = null;    // قناة الحكم / اللاعب
let dpLocalChannel = null;  // قناة البروجكتر (منفصلة لتجنب التعارض)
...
/* قناة منفصلة للبروجكتر حتى لا تطغى على قناة الحكم */
function initDisplayChannel(code, onMessage) {
  try {
    if (dpLocalChannel) dpLocalChannel.close();
    dpLocalChannel = new BroadcastChannel('hexgame-' + code);
    dpLocalChannel.onmessage = (e) => onMessage(e.data);
    return true;
  } catch (e) { return false; }
}

function closeDisplayChannel() {
  if (dpLocalChannel) { dpLocalChannel.close(); dpLocalChannel = null; }
}
...
/* ══════ لعبة محلية ══════ */
let LG = {
  teamA: 'الفريق الأحمر', teamB: 'الفريق الأخضر', roundsToWin: 2, round: 1,
  pts: { a: 0, b: 0 }, rndWins: { a: 0, b: 0 },
  letters: [], owner: Array(N).fill(null), activeCell: null, activeQ: null, timerInt: null
};
...
function lHexClick(i) {
  if (LG.owner[i]) return; LG.activeCell = i; LG.activeQ = pickQ(LG.letters[i]); if (!LG.activeQ) return;
...
function lNextQ() {
  ...
  LG.activeQ = pickQ(LG.letters[LG.activeCell]); if (!LG.activeQ) return;
  document.getElementById('lq-text').innerText = LG.activeQ.q;
  document.getElementById('lq-ans').innerText = '✅ الإجابة: ' + LG.activeQ.a;
  document.getElementById('lq-ans').style.display = 'none';
  clearInterval(LG.timerInt); lHexClick(LG.activeCell);
}

function lMark(team) {
  if (LG.activeCell === null) return; LG.owner[LG.activeCell] = team; LG.pts[team]++; clearInterval(LG.timerInt);
  lCloseModal(); renderHexBoard(document.getElementById('local-board'), LG.letters, LG.owner, null, lHexClick);
  lUpdateUI();
...
function lCloseModal() {
  clearInterval(LG.timerInt); document.getElementById('lv-ov').classList.remove('on');
  document.getElementById('lv-qm').classList.remove('on'); LG.activeCell = null;
  renderHexBoard(document.getElementById('local-board'), LG.letters, LG.owner, null, lHexClick);
}

function lShowWinner(team) {
  LG.rndWins[team]++; lUpdateUI();
  ...
  document.getElementById('local-win').classList.add('on'); confetti(team);
  if (LG.rndWins[team] >= LG.roundsToWin) document.getElementById('lw-sb').innerText = '🏆 فاز باللعبة كاملة!';
}
...
function jPub() {
  return {
    ...
    board: JS.letters.map((l, i) => ({ letter: l, owner: JS.owner[i] })),
    activeCell: JS.activeCell, phase: JS.phase, bzWinner: JS.bzWinner,
    timerSecs: JS.timerSecs, pts: JS.pts,
    showQuestion: JS.showQuestion, showAnswer: JS.showAnswer, activeQ: JS.activeQ
  };
}
...
function jHexClick(i) {
  ...
  jClearTimer(); renderHexBoard(document.getElementById('judge-board'), JS.letters, JS.owner, i, jHexClick);
  jRenderBz();
...
function jStartTimer(s) {
  jClearTimer(); JS.timerSecs = s; jRenderBz();
  jBcast({ type: 'tick', secs: JS.timerSecs });
  JS.timerInt = setInterval(() => {
    JS.timerSecs--; jRenderBz();
    jBcast({ type: 'tick', secs: JS.timerSecs });
...
function jTimerEnd() {
  if (JS.phase === 'won' && JS.bzWinner) {
    ...
    JS.phase = 'open'; JS.bzWinner = null;
    jStartTimer(10);
...
function jShowAns() {
  ...
  const ab = document.getElementById('j-ans'); ab.innerText = '✅ ' + JS.activeQ.a; ab.style.display = 'block';
}

function jCorrect(team) {
  if (JS.activeCell === null) return;
  JS.owner[JS.activeCell] = team; JS.pts[team]++;
  jClearTimer(); JS.phase = 'idle'; JS.bzWinner = null; JS.activeCell = null;
...
function jNextQ() {
  if (JS.activeCell === null) return;
  JS.activeQ = pickQ(JS.letters[JS.activeCell]); if (!JS.activeQ) return;
  document.getElementById('j-qbox').innerText = JS.activeQ.q;
  document.getElementById('j-ans').style.display = 'none';
  jBcast({ type: 'show_q', q: JS.activeQ.q });
}
...
function jReopen() {
  if (JS.activeCell === null) return;
  JS.phase = 'open'; JS.bzWinner = null; jClearTimer(); jRenderBz();
...
function jSkip() {
  ...
  renderHexBoard(document.getElementById('judge-board'), JS.letters, JS.owner, null, jHexClick);
  jRenderBz();
  jBcast({ type: 'skip' }); jBcast({ type: 'state', s: jPub() });
}

function jNewRound() {
  JS.round++; jInitBoard(); jUpdateUI();
  jBcast({ type: 'new_round', round: JS.round }); jBcast({ type: 'state', s: jPub() });
  document.getElementById('judge-win').classList.remove('on');
}

function jShowWinner(team) {
  JS.rndWins[team]++; jUpdateScores();
  ...
  document.getElementById('judge-win').classList.add('on'); confetti(team);
...
function jToggleQuestion() {
  ...
  jBcast({ type: 'toggle_question', show: JS.showQuestion, q: JS.activeQ ? JS.activeQ.q : '' });
}
...
function jRenderPlayers() {
  const pl = document.getElementById('j-players'); if (!pl) return;
  ...
  pl.innerHTML = JS.players.map(p => {
    ...
    return '<div class="plitem p' + p.team + '"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:' + teamColor + ';margin-left:4px;"></span><span style="flex:1;">' + p.name + '</span><span style="opacity:.6;font-size:.72rem;">' + (p.team === 'a' ? JS.teamA : JS.teamB) + '</span><button class="plkick" onclick="jKickPlayer(\'' + p.id + '\')" title="طرد اللاعب">✕</button></div>';
  }).join('');
}

function jKickPlayer(playerId) {
  const player = JS.players.find(p => p.id === playerId); if (!player) return;
  ...
  jRenderPlayers(); jBcast({ type: 'state', s: jPub() });
}
...
function pJoin() {
  ...
  if (!name) { err.innerText = 'يرجى إدخال اسمك'; err.style.color = '#e74c3c'; err.style.display = 'block'; setTimeout(() => err.style.display = 'none', 2500); return; }
  if (!pSelT) { err.innerText = 'يرجى اختيار فريقك'; err.style.color = '#e74c3c'; err.style.display = 'block'; setTimeout(() => err.style.display = 'none', 2500); return; }
  if (!code) { err.innerText = 'يرجى إدخال كود الجلسة'; err.style.color = '#e74c3c'; err.style.display = 'block'; setTimeout(() => err.style.display = 'none', 2500); return; }
  PM = { name, team: pSelT, id: 'p_' + Math.random().toString(36).substr(2, 8) };
  pCode = code;
  /* إظهار حالة الاتصال على شاشة الانضمام */
  err.style.color = 'var(--GOLD)';
  err.innerText = '⏳ جارٍ الاتصال...';
  err.style.display = 'block';
  /* تهيئة قناة BroadcastChannel للاتصال المحلي (نفس الجهاز) */
  pUseLocal = initLocalChannel(code, (data) => pOnData(data));
  if (pUseLocal) {
    localSend({ type: 'join', name: PM.name, team: PM.team, id: PM.id });
  }
  /* تعطيل زر الانضمام لمنع الضغط المزدوج */
  const joinBtn = document.querySelector('.pjoin');
  if (joinBtn) { joinBtn.disabled = true; joinBtn.style.opacity = '0.6'; }
...
function connectPlayer(code) {
  ...
  pPeer.on('open', () => {
    ...
    pConn.on('open', () => {
      ...
      /* الانتقال لشاشة الانتظار عند الاتصال الفعلي */
      pShowWait();
    });
    pConn.on('data', pOnData);
    pConn.on('error', e => {
      const errEl = document.getElementById('perr');
      if (errEl) { errEl.style.color = '#e74c3c'; errEl.innerText = 'خطأ في الاتصال: ' + (e.type || e); errEl.style.display = 'block'; }
    });
  });
...
function handlePlayerError(e, code) {
  ...
  /* إعادة تفعيل زر الانضمام عند الخطأ */
  const joinBtn = document.querySelector('.pjoin');
  ...
  if (currentServerIndex < PEER_SERVERS.length - 1) { currentServerIndex++; connectPlayer(code); }
    ...
    /* العودة لشاشة الانضمام وإظهار الخطأ */
    go('player-join');
    const errEl = document.getElementById('perr');
    if (errEl) { errEl.style.color = '#e74c3c'; errEl.innerText = '❌ تعذّر الاتصال — تحقق من الكود وأعد المحاولة'; errEl.style.display = 'block'; }
    pRetryTimeout = setTimeout(() => connectPlayer(code), 5000);
  }
}
...
function pOnData(d) {
  ...
  if (d.type === 'state') {
    pGS = d.s;
    /* إذا كانت البيانات وصلت قبل فتح PeerJS (عبر BroadcastChannel)، انقل اللاعب لشاشة الانتظار */
    const cur = document.querySelector('.scr.on');
    if (cur && (cur.id === 'player-join' || cur.id === 'home')) { pShowWait(); }
    renderMiniBoard(pGS.board);
    return;
  }
  ...
  if (d.type === 'toggle_question') { const qc = document.getElementById('pbz-q'); if (d.show && pGS && pGS.activeQ) qc.innerText = '❓ ' + pGS.activeQ.q; if (qc) qc.style.display = d.show ? 'block' : 'none'; }
  if (d.type === 'toggle_answer') { const ac = document.getElementById('pbz-ans'); if (d.show) ac.innerText = '✅ الإجابة: ' + d.answer; if (ac) ac.style.display = d.show ? 'block' : 'none'; }
}

function renderMiniBoard(board) {
  if (!board) return;
  const wrap = document.getElementById('miniBoardWrap'); if (!wrap) return;
  wrap.innerHTML = ''; const scale = 0.55;
  ...
  board.forEach((c, i) => {
    const { x, y, w, h } = hexPos(i, scale);
    const d = document.createElement('div'); d.className = 'mini-hex';
    if (c.owner === 'a') d.classList.add('a');
    else if (c.owner === 'b') d.classList.add('b');
    d.style.cssText = 'left:' + x + 'px;top:' + y + 'px;width:' + w + 'px;height:' + h + 'px;font-size:' + (w * .35) + 'px;';
...
function pPressBz(e) {
  ...
  if (!pBzEnabled) return; pBzEnabled = false;
...
function pSetRes(nm, sub, secs, color) {
  const rc = document.getElementById('pres'); rc.style.display = 'block';
  ...
  clearInterval(pCdI); let t = secs;
  document.getElementById('pres-cnt').innerText = t;
  pCdI = setInterval(() => {
    t--; document.getElementById('pres-cnt').innerText = Math.max(0, t);
    if (t <= 0) clearInterval(pCdI);
  }, 1000);
}
...
function togglePanel(side) {
  if (side === 'left') {
    const p = document.getElementById('judge-left-panel'), b = document.getElementById('toggle-left-panel');
    if (p) { p.classList.toggle('collapsed'); if (b) b.textContent = p.classList.contains('collapsed') ? '▶' : '☰'; }
  } else if (side === 'right') {
    const p = document.getElementById('judge-right-panel'), b = document.getElementById('toggle-right-panel');
    if (p) { p.classList.toggle('collapsed'); if (b) b.textContent = p.classList.contains('collapsed') ? '◀' : '☰'; }
  } else if (side === 'display-left') {
    const p = document.getElementById('display-left-panel'), b = document.getElementById('toggle-display-left');
    if (p) { p.classList.toggle('collapsed'); if (b) b.textContent = p.classList.contains('collapsed') ? '▶' : '☰'; }
  } else if (side === 'display-right') {
    const p = document.getElementById('display-right-panel'), b = document.getElementById('toggle-display-right');
    if (p) { p.classList.toggle('collapsed'); if (b) b.textContent = p.classList.contains('collapsed') ? '◀' : '☰'; }
  }
}

function toggleSection(h) {
  const s = h.parentElement; if (s) s.classList.toggle('section-collapsed');
}

/* ══════ QR والروابط ══════ */
function getPlayerURL() { return location.href.split('?')[0] + '?mode=player&code=' + JS.code; }
function getDisplayURL() { return location.href.split('?')[0] + '?mode=display&code=' + JS.code; }

function genQR() {
  const url = getPlayerURL();
  const urlEl = document.getElementById('qr-url'); if (urlEl) urlEl.innerText = url;
  ['qr-sm', 'qr-lg', 'display-qr'].forEach(id => {
    const el = document.getElementById(id); if (!el) return;
    el.innerHTML = '';
    const sz = id === 'display-qr' ? 160 : (id === 'qr-sm' ? 70 : 230);
    try { new QRCode(el, { text: url, width: sz, height: sz, colorDark: '#1a1a2e', colorLight: '#ffffff' }); } catch (e) {}
  });
...
function copyDisplayLink() { const u = getDisplayURL(); navigator.clipboard.writeText(u).then(() => showToast('تم النسخ', 'success')).catch(() => prompt('انسخ:', u)); }
function copyDisplayLinkOnly() { copyDisplayLink(); }
...
function showDisplayScreen() {
  ...
  go('display'); startDisplay(code.trim().toUpperCase());
}
...
function startDisplay(code) {
  ...
  /* استخدم قناة منفصلة للبروجكتر لتجنب تلوث قناة الحكم */
  dpUseLocal = initDisplayChannel(code, (data) => dpOnData(data));
  /* عرض الكود على شاشة البروجكتر */
...
function dpOnData(d) {
  ...
  if (d.type === 'show_q') { dpPendingQ = d.q; }
  if (d.type === 'buzzer_won') { dpShowBz(d.winner); if (dpPendingQ) { const qd = document.getElementById('dp-q'); qd.innerText = '❓ ' + dpPendingQ; qd.style.display = 'flex'; dpPendingQ = null; } }
  ...
  if (d.type === 'toggle_question') { if (d.show && d.q) { const qd = document.getElementById('dp-q'); qd.innerText = '❓ ' + d.q; qd.style.display = 'flex'; } else { document.getElementById('dp-q').style.display = 'none'; } }
  if (d.type === 'toggle_answer') { const ae = document.getElementById('dp-ans'); if (d.show) ae.innerText = '✅ الإجابة: ' + d.answer; ae.style.display = d.show ? 'block' : 'none'; }
}
...
function dpApply(s) {
  ...
  if (rndEl && s.round) { const t = ['', 'الأولى', 'الثانية', 'الثالثة', 'الرابعة', 'الخامسة']; rndEl.innerText = 'الجولة ' + (t[s.round] || s.round); }
  dpBrd = s.board || [];
  const letters = dpBrd.map(c => c.letter);
  const owner = dpBrd.map(c => c.owner);
  renderHexBoard(document.getElementById('display-board'), letters, owner, s.activeCell, null);
  if (s.showAnswer && s.activeQ) { document.getElementById('dp-ans').innerText = '✅ الإجابة: ' + s.activeQ.a; document.getElementById('dp-ans').style.display = 'block'; }
  else { document.getElementById('dp-ans').style.display = 'none'; }
  if (s.activeQ) dpPendingQ = s.activeQ.q;
}

function dpShowBz(w) {
  const col = 'var(--' + (w.team === 'a' ? 'A' : 'B') + ')';
  document.getElementById('dp-bz').innerHTML = '<div style="text-align:center"><div style="font-size:1.4rem;font-weight:900;color:' + col + '">⚡ ' + w.name + '</div><div style="font-size:2.4rem;font-weight:900;font-family:monospace;color:' + col + '" id="dp-cd">5</div></div>';
}

function dpShowSecond(team) {
  const col = 'var(--' + (team === 'a' ? 'A' : 'B') + ')';
  document.getElementById('dp-bz').innerHTML = '<div style="text-align:center"><div style="font-size:1.1rem;font-weight:900;color:' + col + '">🔥 الفرصة الثانية</div><div style="font-size:2rem;font-weight:900;font-family:monospace;color:' + col + '" id="dp-cd">10</div></div>';
}
...
function setDiff(btn, mode) {
  const c = mode === 'j' ? document.getElementById('jdiff-filter') : document.getElementById('ldiff-filter');
  c.querySelectorAll('.diff-btn').forEach(b => b.className = 'diff-btn');
  const d = btn.dataset.diff;
  btn.classList.add('active-' + (d === 'all' ? 'all' : d));
  GAME_DIFF = d;
}

function setCat(btn, mode) {
  const c = mode === 'j' ? document.getElementById('jcat-filter') : document.getElementById('lcat-filter');
  c.querySelectorAll('.cat-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  GAME_CAT = btn.dataset.cat;
}

function setLiveDiff(btn) {
  const c = document.getElementById('j-live-diff');
  c.querySelectorAll('.diff-btn').forEach(b => b.className = 'diff-btn');
  const d = btn.dataset.diff;
  btn.classList.add('active-' + (d === 'all' ? 'all' : d));
  LIVE_DIFF = d;
...
function pickQ2(letter, diffOverride) {
  ...
  if (diff !== 'all') levels = [diff];
  let pool = [];
  for (const lvl of levels) {
    const qs = QBANK2[letter][lvl] || [];
    for (let i = 0; i < qs.length; i++) {
      const q = qs[i];
      if (GAME_CAT === 'all' || q.cat === GAME_CAT) pool.push({ ...q, _lvl: lvl, _idx: i, _letter: letter });
    }
  }
  if (!pool.length) return null;
  const usedKey = letter + '_' + diff + '_' + GAME_CAT;
  if (!UQ2[usedKey]) UQ2[usedKey] = new Set();
  const used = UQ2[usedKey];
  let available = pool.filter((_, i) => !used.has(i));
  if (!available.length) { used.clear(); available = pool; }
...
pickQ = function (letter) {
  ...
  if (typeof QBANK2 !== 'undefined' && QBANK2[letter] && GAME_DIFF === 'all' && GAME_CAT === 'all') {
    if (Math.random() > 0.5) { const q2 = pickQ2(letter); if (q2) return q2; }
  }
  return _origPickQ(letter);
};

function lShowDiffBadge(q) {
  const badge = document.getElementById('lq-diff-badge'), catTag = document.getElementById('lq-cat-badge');
  if (!badge || !catTag) return;
  if (q && q._lvl) {
    const map = { easy: ['🟢 سهل', 'diff-easy'], medium: ['🟡 متوسط', 'diff-medium'], hard: ['🔴 صعب', 'diff-hard'], expert: ['⭐ خبير', 'diff-hard'] };
    const [label, cls] = map[q._lvl] || ['', ''];
    badge.className = 'diff-indicator ' + cls;
    badge.textContent = label;
    catTag.textContent = q.cat || '';
  } else {
    badge.className = 'diff-indicator';
    badge.textContent = '';
    catTag.textContent = '';
  }
}
...
(function init() {
  ...
  if (mode === 'player') {
    ...
    if (code) {
      ...
      const savedName = localStorage.getItem('playerName_' + code);
  ...
  // إعادة رسم اللوحة عند تغيير الحجم
...
function lUpdateUI() {
  document.getElementById('lsa-n').innerText = LG.teamA; document.getElementById('lsb-n').innerText = LG.teamB;
  document.getElementById('lsa-p').innerText = LG.pts.a; document.getElementById('lsb-p').innerText = LG.pts.b;
  document.getElementById('lsa-w').innerText = 'جولات: ' + LG.rndWins.a; document.getElementById('lsb-w').innerText = 'جولات: ' + LG.rndWins.b;
  const t = ['', 'الأولى', 'الثانية', 'الثالثة', 'الرابعة', 'الخامسة'];
  document.getElementById('lrlbl').innerText = 'الجولة ' + (t[LG.round] || LG.round);
}
...
function handleJudgeError(e) {
  ...
  alert('⚠️ مشكلة في الاتصال بالشبكة. جارٍ إعادة المحاولة...');
}
...
function jNextQ() {
  ...
  document.getElementById('j-qbox').innerText = JS.activeQ.q;
  document.getElementById('j-ans').style.display = 'none';
  // إرسال السؤال الجديد للبروجكتر واللاعبين
  jBcast({ type: 'show_q', q: JS.activeQ.q });
}
...
function jUpdateUI() {
  document.getElementById('ja-n').innerText = JS.teamA; document.getElementById('jb-n').innerText = JS.teamB;
  document.getElementById('jb-ca').innerText = '✅ صح (' + JS.teamA.substr(0, 7) + ')'; document.getElementById('jb-cb').innerText = '✅ صح (' + JS.teamB.substr(0, 7) + ')';
...
function genQR() {
  ...
  ['qr-sm', 'qr-lg'].forEach(id => { const el = document.getElementById(id); if (!el) return; el.innerHTML = ''; const sz = id === 'qr-sm' ? 70 : 230; try { new QRCode(el, { text: url, width: sz, height: sz, colorDark: '#1a1a2e', colorLight: '#ffffff' }); } catch (e) {} });
}
...
function connectPlayer(code) {
  ...
  pPeer.on('open', () => {
    pConn = pPeer.connect('hexgame-' + code, { reliable: true });
    pConn.on('open', () => { pConn.send({ type: 'join', name: PM.name, team: PM.team, id: PM.id }); pShowWait(); });
    pConn.on('data', pOnData);
    pConn.on('error', e => { document.getElementById('perr').innerText = 'خطأ: ' + (e.type || e); document.getElementById('perr').style.display = 'block'; });
  });
  pPeer.on('error', e => handlePlayerError(e, code));
  pPeer.on('disconnected', () => { pPeer.reconnect(); });
}
...
window.retryPlayerConnection = function () { if (pPeer) pPeer.destroy(); currentServerIndex = 0; connectPlayer(document.getElementById('pcode').value.trim().toUpperCase()); };
...
function pOnData(d) {
  if (d.type === 'kicked') { alert('لقد تم طردك من اللعبة من قبل الحكم.'); pPeer.destroy(); go('player-join'); return; }
...
function renderMiniBoard(board) {
  ...
  wrap.innerHTML = ''; const scale = 0.55;
  const container = document.createElement('div'); container.style.position = 'relative';
  const { W, H } = boardDims(scale); container.style.width = W + 'px'; container.style.height = H + 'px';
  wrap.appendChild(container);
  board.forEach((c, i) => {
    const { x, y, w, h } = hexPos(i, scale);
    const d = document.createElement('div'); d.className = 'mini-hex';
    if (c.owner === 'a') d.classList.add('a'); else if (c.owner === 'b') d.classList.add('b');
    d.style.cssText = 'left:' + x + 'px;top:' + y + 'px;width:' + w + 'px;height:' + h + 'px;font-size:' + (w * .35) + 'px;';
    d.innerText = c.letter; container.appendChild(d);
  });
}

function pActivateBz() { pBzEnabled = true; go('player-bz'); const btn = document.getElementById('main-bz'); btn.className = 'bzbtn bz' + PM.team + ' rdy'; btn.innerHTML = '<span class="bzic">⚡</span><span class="bzlbl">اضغط الآن!</span>'; document.getElementById('pres').style.display = 'none'; document.getElementById('pbz-st').innerText = '⚡ اضغط البازر بأسرع ما يمكن!'; document.getElementById('pbz-st').style.color = 'var(--GOLD)'; document.getElementById('pbz-q').style.display = 'none'; document.getElementById('pbz-ans').style.display = 'none'; }
...
function pResetWait() { pBzEnabled = false; clearInterval(pCdI); go('player-wait'); document.getElementById('pwmsg').innerText = 'في انتظار اختيار سؤال جديد...'; document.getElementById('pbz-q').style.display = 'none'; document.getElementById('pbz-ans').style.display = 'none'; document.getElementById('pres').style.display = 'none'; }
...
function dpApply(s) {
  if (!s) return;
  document.getElementById('dp-an').innerText = s.teamA || 'الفريق أ'; document.getElementById('dp-bn').innerText = s.teamB || 'الفريق ب';
  document.getElementById('dp-as').innerText = s.pts ? s.pts.a : 0; document.getElementById('dp-bs').innerText = s.pts ? s.pts.b : 0;
  dpBrd = s.board || []; const letters = dpBrd.map(c => c.letter); const owner = dpBrd.map(c => c.owner);
  renderHexBoard(document.getElementById('display-board'), letters, owner, s.activeCell, null);
  if (s.showAnswer && s.activeQ) { document.getElementById('dp-ans').innerText = '✅ الإجابة: ' + s.activeQ.a; document.getElementById('dp-ans').style.display = 'block'; }
  else { document.getElementById('dp-ans').style.display = 'none'; }
  if (s.activeQ) dpPendingQ = s.activeQ.q;
...
function pickQ2(letter, diffOverride) {
  ...
  for (const lvl of levels) {
    ...
    for (let i = 0; i < qs.length; i++) {
      const q = qs[i];
      if (GAME_CAT === 'all' || q.cat === GAME_CAT) {
        pool.push({ ...q, _lvl: lvl, _idx: i, _letter: letter });
      }
    }
  }
  if (!pool.length) return null;
...
// الإصلاح الحاسم: Function Expression بدلاً من Function Declaration
const _origPickQ = pickQ;
pickQ = function (letter) {
  if (QBANK2[letter] && (GAME_DIFF !== 'all' || GAME_CAT !== 'all')) {
    const q2 = pickQ2(letter);
    if (q2) return q2;
  }
  if (QBANK2[letter] && GAME_DIFF === 'all' && GAME_CAT === 'all') {
...
function lShowDiffBadge(q) {
  ...
  if (!badge || !catTag) return;
  if (q && q._lvl) {
    const map = { easy: ['🟢 سهل', 'diff-easy'], medium: ['🟡 متوسط', 'diff-medium'], hard: ['🔴 صعب', 'diff-hard'] };
    const [label, cls] = map[q._lvl] || ['', ''];
    badge.className = 'diff-indicator ' + cls;
    badge.textContent = label;
    catTag.textContent = q.cat || '';
